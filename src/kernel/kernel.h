    ...
    public:
        ...
        void read_file(TypedGlobalTable<K, V, V, D>* table) {
            ...
            while (inFile.getline(linechr, 2024000)) { //read a line of the input file, ensure the buffer is large enough
                ...
                D data;
                string line(linechr);
                maiter->iterkernel->read_data(line, key, final_v, value, delta, data); //invoke api, get the value of key field and data field
                ...
                table->put(key, delta, value, final_v, data); //initialize a row of the state table (a node)
    ...
    public:
        ...
        void run_loop(TypedGlobalTable<K, V, V, D>* a) {
            ...
            while (true) {
                ...
                //            NetworkThread::Get()->work_barrier(); //barrier for workers
    ...
    public:
        ...
        void dump(TypedGlobalTable<K, V, V, D>* a) {
            ...
            while (!it->done()) {
                ...
                totalF1 += it->value1();
                totalF2 += it->value2();
                File << it->key() << "\t" << it->value2() << "\t" << it->value1() << "\n";
