...
void MutableGlobalTableBase::termcheck() {
    ...
    for (int i = 0; i < partitions_.size(); ++i) {   
      if (is_local_shard(i)) {
        ...
        double partdiff = 0;
        long partUpdates;
        t->termcheck(StringPrintf("snapshot/iter%d-part%d", snapshot_index, i), &partUpdates, &partF2, &partdiff);
    ...
    if(helper()){
        helper()->SendTermcheck(snapshot_index, total_updates, total_current, total_diff);
    }
