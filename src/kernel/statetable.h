...
#pragma pack(push, 1)
  struct Bucket {
    ...
    V1 v1; // delta value
    V2 v2; // vertex value
    V2 final_v; //for experiment final vertex value
    V3 v3; // neighbor
    V1 priority;
    bool in_use;
  };
...
public:
  ...
  struct Iterator : public TypedTableIterator<K, V1, V2, V3> {
	...
	virtual ~Iterator(){}
        ...
        V2& final_v(){return parent_.buckets_[pos].final_v;}
  ...
  struct ScheduledIterator : public TypedTableIterator<K, V1, V2, V3> {
        ...
        if(parent_.entries_ <= sample_size){
            ...
            if(!bfilter) b_no_change = false;
                ...
                */
                ...
                V1 threshold = abs(parent_.buckets_[sampled_pos[cut_index]].priority);
                ...
                if(cut_index==0 || parent_.buckets_[sampled_pos[0]].priority == threshold){
                    ...
                    for(i=0; i<parent_.size_; i++){
                        ...
                        if(abs(parent_.buckets_[i].priority) >= threshold){
                            scheduled_pos.push_back(i); 
                        }
                    }
                }else{
	...
	virtual ~ScheduledIterator(){}
        ...
        V2& final_v() {return parent_.buckets_[scheduled_pos[pos]].final_v;}
  ...
  void put(const K& k, const V1& v1, const V2& v2, const V2& final_v, const V3& v3);
  ...
  bool compare_priority(int i, int j) {
      return abs(buckets_[i].priority) > abs(buckets_[j].priority);
  ...
  TableIterator *get_iterator(TableHelper* helper, bool bfilter) {
      ...
      while(iter->b_no_change){
          ...
          if(trial >= 10){
              ...
              helper->SendTermcheck(-1, total_updates, total_curr, total_diff);
  ...
  TableIterator *schedule_iterator(TableHelper* helper, bool bfilter) {
      ...
      while(iter->b_no_change){      
          ...
          if(trial >= 10){
              ...
              VLOG(1) << "send term check since many times trials " << total_curr << " and perform a pass of the current table " << total_diff;
              helper->SendTermcheck(-1, total_updates, total_curr, total_diff);
  ...
  void serializeToSnapshot(const string& f, long *updates, double *totalF2, double *totalDiff);
...
StateTable<K, V1, V2, V3>::StateTable(int size)
  : buckets_(0), entries_(0), size_(0), total_curr(0), total_updates(0), total_diff(0) {
  clear();
...
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::serializeToSnapshot(const string& f, long* updates, double* totalF2, double* totalDiff) {
  total_curr = 0;
  total_diff = 0;
  EntirePassIterator* entireIter = new EntirePassIterator(*this);
  double curr = static_cast<double>(((TermChecker<K, V2>*)info_.termchecker)->estimate_prog(entireIter, &total_curr, &total_diff));
  delete entireIter;
...
void StateTable<K, V1, V2, V3>::resize(int64_t size) {
 ...
 // LOG(INFO) << "Rehashing.vd.. " << entries_ << " : " << size_ << " -> " << size;
  ...
  for (int i = 0; i < old_b.size(); ++i) {
    if (old_b[i].in_use) {    
      put(old_b[i].k, old_b[i].v1, old_b[i].v2, old_b[i].final_v,  old_b[i].v3);
...
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::put(const K& k, const V1& v1, const V2& v2, const V2& final_v, const V3& v3) {
  int start = bucket_idx(k);
  int b = start;
  bool found = false;
