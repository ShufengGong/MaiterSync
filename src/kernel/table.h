...
struct TableHelper {
  ...
  virtual void SendTermcheck(int index, long updates, double current, double diff) = 0;
};
...
struct IterateKernel : public IterateKernelBase {
  ...
  virtual void read_data(string& line, K& k, V& final_v, V& vlaue, V& delta, D& data) = 0;
...
struct TermChecker : public TermCheckerBase {
    ...
    virtual double estimate_prog(LocalTableIterator<K, V>* table_itr, double* total_value, double* totalDiff) = 0;
...
struct TermCheckers {
  struct Diff : public TermChecker<K, V> {
    ...
    double estimate_prog(LocalTableIterator<K, V>* statetable, double* total_value, double* totalDiff){
        ...
        while(!statetable->done()){
            ...
            if(statetable->value2() != defaultv){
                *total_value += static_cast<double>(statetable->value2());
                *totalDiff += static_cast<double>(abs(statetable->value2() - statetable->final_v()));
            }
        }
        return 0;
    }
...
public:
  ...
  virtual void put(const K &k, const V1 &v1, const V2 &v2, const V2 &final_v, const V3 &v3) = 0;
...
class Snapshottable {
public:
  virtual void serializeToSnapshot(const string& f, long* updates, double* totalF2, double* totalDiff) = 0;
};
