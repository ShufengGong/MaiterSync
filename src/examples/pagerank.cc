...
struct PagerankIterateKernel : public IterateKernel<int, float, vector<int> > {
    ...
    void read_data(string& line, int& k, float& final_v, float& value, float& delta, vector<int>& data){
        string linestr(line);
        ...
        pos = remain.find("\t");
        if(pos == -1) return;
        final_v = boost::lexical_cast<float>(remain.substr(0, pos));
        
        remain = remain.substr(pos + 1);
        pos = remain.find("\t");
        if(pos == -1) return;
        value = boost::lexical_cast<float>(remain.substr(0, pos));
        
        
        remain = remain.substr(pos + 1);
        pos = remain.find("\t");
        if(pos == -1) return;
        delta = boost::lexical_cast<float>(remain.substr(0, pos));
        
        vector<int> linkvec;
        string links = remain.substr(pos+1);
